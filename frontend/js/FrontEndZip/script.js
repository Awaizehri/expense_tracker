...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addTransactionForm) {
        ...
        //moving to database from local storage
        ...
        if (editId) {
            (async function loadEditData(){
                try {
                    ...
                    //find the specific transaction using the URL ID
                    ...
                    if (editItem){
                        ...
                        // Format the MongoDB ISO date to fit the HTML date picker (YYYY-MM-DD)
...
// 3. Handle Form Submission (POST vs PUT)
        addTransactionForm.addEventListener('submit', async function(e) {
            ...
            try {
                if (editId) {
                    // UPDATE: Use the URL ID to send a PUT request to MongoDB
                    ...
                    // CREATE: Send a POST request to MongoDB
...
async function editTransaction(id) {
    try {
        ...
        // Find the specific transaction -. for mongodb (-id === id )
        const transaction = transactions.find(tnx => tnx._id === id);
